...
- 新增功能
    ...
    - 新增一款视频播放器，书源支持视频类型(支持多线路)
    - 本地TXT目录规则支持用js更高效的处理，支持index、prevTitle、prevLength变量
    - 净化规则使用js时支持book，chapter
    - 支持书源监听加入书架，移除书架，分享按钮，保存进度等事件并进行函数回调，支持自定义界面按钮
    - 支持关闭启动时的欢迎页，启动速度更快
    - 支持书源迅速校验源地址（注意源地址不可访问和书源失效没有绝对的关系），支持校验书源不改变源注释
    - 正文内容搜索支持正则表达式
...
- 功能变动
    ...
    - 订阅源内的查看历史记录按钮只能看到自己的历史记录，外面主页订阅的历史按钮查看所有记录
    - 去掉主页我的界面的亮条分割线，让观感更舒适
    - 订阅源网页观看视频时，支持重力控制横屏的翻转，需要js主动调用方向控制函数。不再强制启用全方向重力旋转。（不会js的建议使用视频类订阅源）
- 功能完善
    - 订阅源支持显示识别更新状态
    - 字数显示能智能识别在线字数
    - 图片链接不计入本地字数统计
